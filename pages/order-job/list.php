...
$bpr_arr = array();
$manages_arr = array();
$all_bookings = $manageObj->fetch_all_bookingboat('all', $today, $search_status = 'all', $search_agent = 'all', $search_product = 'all', $search_voucher_no = '', $refcode = '', $name = '', $hotel = '', $search_boat = 'all', $search_guide = 'all', 0);
                                                ...
                                                if (!empty($bo_id[$manage_id[$m]])) {
                                                    for ($i = 0; $i < count($bo_id[$manage_id[$m]]); $i++) {
                                                        if (in_array($bo_id[$manage_id[$m]][$i], $booking_id_arr) == false) {
                                                            ...
                                                            // $check_in = $manageObj->get_values('id', 'check_in', 'booking_id = ' . $id . ' AND type = 1', 0);
                                                ?>
                                                            <tr>
