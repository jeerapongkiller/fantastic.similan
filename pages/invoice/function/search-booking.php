    ...
    if (!empty($bookings)) {
        foreach ($bookings as $booking) {
            ...
            if (in_array($booking['id'], $first_booking) == false) {
                ...
                // $arr_bo[$booking['id']]['total'] = $booking['bp_private_type'] == 1 ? ($booking['bp_adult'] * $booking['rates_adult']) + ($booking['bp_child'] * $booking['rates_child']) : $booking['rates_private'];
            }
            ...
            if (in_array($booking['bpr_id'], $first_bpr) == false) {
                ...
                $rate_adult[$booking['id']][] = !empty($booking['rates_adult']) ? $booking['rates_adult'] : 0;
                $rate_child[$booking['id']][] = !empty($booking['rates_child']) ? $booking['rates_child'] : 0;
                ...
                $total_rates[$booking['id']][] = ($booking['booking_type_id'] == 1) ? ($booking['adult'] * $booking['rates_adult']) + ($booking['child'] * $booking['rates_child']) : $booking['rates_private'];
                ...
                $arr_rates[$booking['id']]['rate_adult'][] = !empty($booking['rates_adult']) && $booking['adult'] > 0 ? $booking['rates_adult'] : '-';
                $arr_rates[$booking['id']]['rate_child'][] = !empty($booking['rates_child']) && $booking['child'] > 0 ? $booking['rates_child'] : '-';
