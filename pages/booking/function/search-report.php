...
if (isset($_POST['action']) && $_POST['action'] == "search" && !empty($_POST['type_date']) && !empty($_POST['type'])) {
    ...
    $first_driver = array();
    $first_book = array();
    $bookings = $bookObj->showlist($_SESSION["supplier"]["id"], 'all', 'all', 'all', 'all', $_POST['type_date'] == 'today' ? $today : $tomorrow, '', '', '');
    if (!empty($bookings)) {
    ...
    <?php if ($_POST['type'] == 'boat') { ?>
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center p-1 ">
                ...
                <h5 class="card-title"><?php echo $_POST['type_date'] == 'today' ? date('j F Y', strtotime($today)) : date('j F Y', strtotime($tomorrow)); ?></h5>
            </div>
