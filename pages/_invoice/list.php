        ...
        <div class="content-body">
            ...
            <section id="basic-tabs-components">
                ...
                <div class="card">
                    ...
                    <form id="invoice-search-form" name="invoice-search-form" method="post" enctype="multipart/form-data">
                        <div class="d-flex align-items-center mx-50 row pt-0 pb-2">
                            ...
                            <div class="col-md-2 col-12">
                                <div class="form-group">
                                    <label for="search_vat">Vat</label>
                                    <select class="form-control select2" id="search_vat" name="search_vat[]" multiple>
                                        <option value="0" selected>No Vat</option>
                                        <?php
                                        $vats = $invObj->showvat();
                                        foreach ($vats as $vat) {
                                        ?>
                                            <option value="<?php echo $vat['id']; ?>" selected><?php echo $vat['name']; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                ...
                <div id="invoice-search-table">
                    ...
                    $first_bpr = array();
                    $first_extar = array();
                    $invoices = $invObj->showlist($search_type = 'all', $search_agent = 'all', $search_invoice = '', $search_booking = '', $search_voucher = '', $search_travel = '', $today, 'all');
                    if (!empty($invoices)) {
                        foreach ($invoices as $invoice) {
                            ...
                            if (in_array($invoice['cover_id'], $first_cover) == false) {
                                ...
                                $due_date[$invoice['comp_id']][] =
                                    (diff_date($today, $invoice['rec_date'])['day'] > 0) ?
                                    '<span class="badge badge-pill badge-light-warning text-capitalized">' . date("j F Y", strtotime($invoice['rec_date'])) . ' (ครบกำหนดชำระในอีก ' . diff_date($today, $invoice['rec_date'])['num'] . ' วัน)</span>' :
                                    '<span class="badge badge-pill badge-light-danger text-capitalized">' . date("j F Y", strtotime($invoice['rec_date'])) . ' (เกินกำหนดชำระมาแล้ว ' . diff_date($today, $invoice['rec_date'])['num'] . ' วัน)</span>';
                            }
                            # --- get value booking --- #
                            if (in_array($invoice['id'], $first_booking) == false) {
                                $first_booking[] = $invoice['id'];
                                $bo_id[$invoice['comp_id']][] = !empty($invoice['id']) ? $invoice['id'] : 0;
                                $travel_date[$invoice['comp_id']][] = !empty($invoice['travel_date']) ? $invoice['travel_date'] : 0;
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                                <tr>
                                                    ...
                                                    <td <?php echo $modal; ?>>
                                                        <?php
                                                        echo $rec_id[$agent_id[$i]][$a] == 0 ?
                                                            $due_date[$agent_id[$i]][$a] :
                                                            '<span class="badge badge-pill badge-light-success text-capitalized">ชำระเรียบร้อยแล้ว</span>';
                                                        ?>
                                                    </td>
        ...
        <div class="modal fade text-left" id="modal-invoice" tabindex="-1" role="dialog" aria-labelledby="myModalLabel16" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                <div class="modal-content">
                    ...
                    <div class="row pl-1" id="div-select-invoice-date">
