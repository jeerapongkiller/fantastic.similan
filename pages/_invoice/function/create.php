...
if (isset($_POST['action']) && $_POST['action'] == "create" && isset($_POST['bo_id'])) {
    ...
    if (count($bo_id) > 1) {
        ...
        $no = 0;
        $response = $response != false && $response > 0 ? $invObj->insert_data($no, $rec_date, $withholding, $note, $branch, $bo_id[0], $payment_id, $vat_id, $currency_id, $cover_id, $bank_account_id, $is_approved) : false;
        # --- insert booking paid --- #
        $response = $response != false && $response > 0 ? $invObj->insert_booking_paid($bo_id[0], 6) : false;
    }
