    ...
    if ($_GET['type'] == 'booking') {
        ...
        if (!empty($agent_id)) {
            for ($i = 0; $i < count($agent_id); $i++) {
                $columnName[] = [$agent_name[$i], array_count_values($comp_id)[$agent_id[$i]], !empty($comp_adult[$agent_id[$i]]) ? array_sum($comp_adult[$agent_id[$i]]) : 0, !empty($comp_child[$agent_id[$i]]) ? array_sum($comp_child[$agent_id[$i]]) : 0, !empty($comp_infant[$agent_id[$i]]) ? array_sum($comp_infant[$agent_id[$i]]) : 0, !empty($comp_foc[$agent_id[$i]]) ? array_sum($comp_foc[$agent_id[$i]]) : 0, !empty($comp_sum[$agent_id[$i]]) ? array_sum($comp_sum[$agent_id[$i]]) : 0, number_format(array_sum($comp_amount[$agent_id[$i]])), number_format(array_sum($comp_revenue[$agent_id[$i]])), number_format(array_sum($comp_amount[$agent_id[$i]]) - array_sum($comp_revenue[$agent_id[$i]])),];
            }
        }
